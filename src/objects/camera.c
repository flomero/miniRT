...
/*   Updated: 2024/07/04 15:24:31 by flfische         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vector3	*calc_lower_left_corner(t_object *camera)
{
	...
	ft_v3_init(&camera->s_camera.ll_corner, camera->pos.x, camera->pos.y,
		camera->pos.z);
	ft_v3_init(&camera->s_camera.hor, u.x, u.y, u.z);
	ft_v3_scalar_ip(&camera->s_camera.hor, camera->s_camera.vp_width);
	ft_v3_init(&camera->s_camera.ver, v.x, v.y, v.z);
	ft_v3_scalar_ip(&camera->s_camera.ver, camera->s_camera.vp_height);
	ft_v3_scalar_ip(&u, camera->s_camera.vp_width / 2.0);
	ft_v3_scalar_ip(&v, camera->s_camera.vp_height / 2.0);
	ft_v3_sub_ip(ft_v3_sub_ip(ft_v3_sub_ip(&camera->s_camera.ll_corner, &u),
			&v), &w);
	return (&camera->s_camera.ll_corner);
}
